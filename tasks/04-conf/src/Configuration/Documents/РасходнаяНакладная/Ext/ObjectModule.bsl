...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если Выборка.Валюта = Справочники.Валюты.РоссийскийРубль Тогда
		КурсВалюты = 1;
	Иначе
		КурсВалюты = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(Дата, Новый Структура("Валюта", Выборка.Валюта)).Курс;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(КурсВалюты) Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не заполнен курс валюты договора";
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
